...
class MainActivity : Activity(), TextToSpeech.OnInitListener {
    ...
    private fun buildUi() {
        ...
        header.addView(TextView(this).apply { text = "☰"; textSize = 27f; gravity = Gravity.CENTER; setTextColor(white); setPadding(10, 6, 6, 6); setOnClickListener { showMenu(it) } }, LinearLayout.LayoutParams(52, 52))
    ...
    private fun saveHistory(value: String) {
        ...
        history.remove(clean); history.add(0, clean)
        while (history.size > 50) history.removeAt(history.lastIndex)
        getPreferences(0).edit().putStringSet("chat_history", history.toSet()).apply()
    }
    private fun showHistory() {
        val values = if (history.isEmpty()) arrayOf("Abhi koi purani chat nahi hai") else history.toTypedArray()
        AlertDialog.Builder(this).setTitle("Chat History").setItems(values) { _, which -> if (history.isNotEmpty()) { input.setText(history[which]); input.setSelection(input.length()) } }.setNegativeButton("Close", null).show()
    }
    ...
    private fun sendMessage(value: String, speakReply: Boolean) {
        ...
        when (val action = BarbieCommandRouter.route(value)) {
            ...
            is WebSearchAction -> { startActivity(Intent(Intent.ACTION_VIEW, BarbieCommandRouter.webSearchUri(action.query))); addAssistantMessage("Search khol di."); status="Ready" }
