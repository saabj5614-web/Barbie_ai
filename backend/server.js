...
async function chat(message) {
  if (OPENROUTER_KEY) { try { const reply = await provider('https://openrouter.ai/api/v1/chat/completions', OPENROUTER_KEY, 'openrouter/free', message); if (reply) return { reply, provider: 'openrouter' }; } catch {} }
  if (HF_TOKEN) { try { const reply = await provider('https://router.huggingface.co/v1/chat/completions', HF_TOKEN, 'openai/gpt-oss-120b:fastest', message); if (reply) return { reply, provider: 'huggingface' }; } catch {} }
